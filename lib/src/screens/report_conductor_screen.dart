...
class ReportConductorScreen extends StatefulWidget {
  const ReportConductorScreen({Key? key}) : super(key: key);

  @override
  _ReportConductorScreenState createState() => _ReportConductorScreenState();
}

class _ReportConductorScreenState extends State<ReportConductorScreen> {
  ...
  Future<void> _loadConductores() async {
    ...
    if (users != null) {
      setState(() {
        conductores = users
            .where((u) => u['tipo_usuario'] == 'conductor')
            .toList();
        isLoading = false;
      });
    } else {
  ...
  Future<void> _downloadAndOpenPDF(int conductorId, String tipoReporte) async {
    ...
    String url = '$baseUrl/reporte-detalle/$conductorId';

    if (tipoReporte == 'mes' && selectedMonth != null) {
      final mes = selectedMonth!.month.toString().padLeft(2, '0');
      final anio = selectedMonth!.year;
      final mesParam = "$anio-$mes"; // Formato YYYY-MM
      url = '$baseUrl/reporte-detalle-mes/$conductorId/$mesParam';
    } else if (tipoReporte == 'personalizado' &&
        startDate != null &&
        endDate != null) {
      final start = startDate!.toIso8601String().split("T").first; // YYYY-MM-DD
      final end = endDate!.toIso8601String().split("T").first; // YYYY-MM-DD
      url = '$baseUrl/reporte-detalle-personalizado/$conductorId/$start/$end';
    }
    ...
    if (response.statusCode == 200) {
      ...
      final file = File('${dir.path}/reporte_${conductorId}.pdf');
  ...
  Widget build(BuildContext context) {
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Reportes Conductor'),
          bottom: const TabBar(
            tabs: [
              Tab(text: 'Diario'),
              Tab(text: 'Mes'),
